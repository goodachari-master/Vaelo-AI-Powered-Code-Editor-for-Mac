...
function knownModel(id) {
	...
	if (!model) { throw new Error('Use Add Model to register a model.'); }
	return model;
}
...
class LocalModels {
	...
	async request(route, body, signal, timeout = 120000) {
		...
		if (!response.ok) {
			...
			const error = new Error(`Ollama HTTP ${response.status}${detail ? ': ' + detail : ''}`);
	...
	async activate(id, previous, signal) {
		...
		if (previous && previous !== id && status.some(model => model.id === previous && model.installed)) {
			try {
				await this.request('/api/generate', { model: previous, keep_alive: 0, stream: false }, signal);
			} catch (error) {
				// A model may be removed between listing it and unloading it.

...
function line(role, text) {
	...
	article.append(label, content); $('thread').append(article); return content;
}
function renderModels() {
	...
	for (const model of models) {
		...
		const status = document.createElement('small'); status.textContent = (active === model.id ? 'SELECTED · ' : '') + (model.kind === 'api' ? model.tested ? 'API · Connection tested' : 'API · Test on activation' : model.loaded ? 'Loaded in memory' : model.installed ? 'Downloaded' : `~${model.downloadGB} GB download`);
		const button = document.createElement('button'); button.textContent = model.installed ? active === model.id ? 'Activate Again' : 'Activate' : 'Download'; button.disabled = busy;
		button.onclick = () => post(model.installed ? 'activate' : 'download', { id: model.id });
...
window.addEventListener('message', event => {
	...
	if (data.type === 'models') {
		...
		$('memory').textContent = `${data.memoryGB} GB system RAM detected. The three featured local models target ~32 GB or more; GPU and context requirements vary.`;
	...
	if (data.type === 'busy') { busy = data.value; for (const id of ['add-api', 'add-link', 'add-file', 'add-installed']) { $(id).disabled = busy; } $('send').disabled = busy; $('clear').disabled = busy; $('propose').disabled = busy; $('stop').disabled = !busy; renderModels(); }
